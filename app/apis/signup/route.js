...
const validateSignup = [
  body('name').isLength({min:3}).withMessage('Name should atleast 3 characters'),
...
export async function POST(request) {
  ...
  try {
    ...
    const db = client.db('delivery-credintial');
    const collection = db.collection('secure-delivery');
